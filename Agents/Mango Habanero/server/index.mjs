...
app.post('/api/submissions/:id/preview', async (req, res) => { try { const { data, error } = await supabase.rpc('mango_list_wing_submissions', {}); if (error) throw error; const row = (data || []).find((item) => item.submission_id === req.params.id); if (!row) return res.status(404).json({ error: 'Submission not found.' }); const path = row.processed_storage_path || row.thumbnail_storage_path || row.original_storage_path; if (!path) return res.status(404).json({ error: 'No preview media is available yet.' }); const expiresIn = Math.min(300, Math.max(30, Number(process.env.MANGO_PREVIEW_SECONDS || 60))); const signed = await supabase.storage.from('wing-submissions').createSignedUrl(path, expiresIn); if (signed.error) throw signed.error; log('signed_preview_generated', { submission_id: req.params.id, expires_in: expiresIn }); res.set('Cache-Control', 'no-store').json({ url: signed.data.signedUrl, expiresIn }); } catch (error) { log('signed_preview_failed', { submission_id: req.params.id }); res.status(502).json({ error: 'Unable to generate a private preview.' }); } });
