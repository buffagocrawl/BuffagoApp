...
def _creative_metadata(content_decision: dict[str, Any], image_pipeline: dict[str, Any] | None) -> dict[str, Any]:
    ...
    metadata = {
        ...
        "copy_source": winner.get("copy_source") or decision_summary.get("copy_source"),
        ...
        "original_video_url": winner.get("original_video_url"),
    ...
    if client is not None:
        ensure_selected_post_candidate(
            ...
            winner_payload={
                ...
                "selected_caption": approved_post.metadata.get("selected_caption") if isinstance(approved_post.metadata, dict) else None,
                ...
                "caption_text": approved_post.metadata.get("caption_text") if isinstance(approved_post.metadata, dict) else approved_post.caption,
                "selected_overlay": approved_post.metadata.get("selected_overlay") if isinstance(approved_post.metadata, dict) else approved_post.overlay_text,
            },
            decision_summary=content_decision.get("decision_summary") if isinstance(content_decision.get("decision_summary"), dict) else {},
            logger=logger,
        )
    ...
    if client is not None and not post_id:
        ...
        if existing_rows:
            ...
            inserted = insert_final_post(
                ...
                original_video_url=approved_post.original_video_url,
                processed_video_url=approved_post.processed_video_url,
                original_storage_path=approved_post.original_storage_path,
                processed_storage_path=approved_post.processed_storage_path,
                overlay_text=approved_post.overlay_text,
                overlay_status=approved_post.overlay_status,
                overlay_error=approved_post.overlay_error,
                selected_caption=approved_post.metadata.get("selected_caption") if isinstance(approved_post.metadata, dict) and approved_post.metadata.get("selected_caption") else approved_post.caption,
                selected_overlay=approved_post.overlay_text,
                caption_text=approved_post.caption,
                copy_source=approved_post.metadata.get("copy_source") if isinstance(approved_post.metadata, dict) else None,
                generated_at=approved_post.metadata.get("generated_at") if isinstance(approved_post.metadata, dict) else None,
                reuse_blocked_reason=approved_post.metadata.get("reuse_blocked_reason") if isinstance(approved_post.metadata, dict) else None,
                openai_used=bool(approved_post.metadata.get("copy_source") == "openai") if isinstance(approved_post.metadata, dict) else None,
                openai_model=approved_post.metadata.get("openai_model") if isinstance(approved_post.metadata, dict) else None,
                fallback_reason=approved_post.metadata.get("fallback_reason") if isinstance(approved_post.metadata, dict) else None,
