...
def build_brand_validation_prompt(
    *,
    ...
    return (
        ...
        f"{_prompt_sections('brand', 'voice', 'content_rules', 'banned_phrases', 'shareable_food_post_rules', 'quality_review')}\n\n"
...
def build_text_generation_prompt(
    *,
    ...
    return (
        ...
        f"Prompt library version: {PROMPT_LIBRARY_VERSION}\n\n"
        f"{_prompt_sections('brand', 'voice', 'content_rules', 'banned_phrases', 'required_ctas', 'shareable_food_post_rules')}\n\n"
        f"Slot prompt:\n{slot_prompt}\n\n"
        f"Content slot: {content_slot}\n\n"
        f"Brand rules:\n{_brand_rules_text(brand_rules)}\n\n"
        f"Privacy-safe internal snapshot:\n{internal_snapshot}\n\n"
        ...
        "- Every caption must include a clear engagement CTA such as Send this to..., Share this with..., Tag..., Comment..., Like..., Reply..., Vote..., Pick..., Choose..., or Make plans...\n"
        "- Favor engagement prompts that invite tagging a friend, commenting, sharing, answering a question, picking a side, or sending it to someone\n"
        "- Use exactly one of these caption styles as the lane for the caption:\n"
        f"{_caption_style_system_text()}\n"
        "- If the creative will also have overlay text, make the overlay reinforce the same CTA and not a different joke\n"
        "- Prefer direct, obvious wording over cleverness\n"
        ...
        "- Avoid stale or awkward phrases like voicemail, understood the assignment, bring napkins, had a job, paid rent, brought receipts, illegal levels, or POV unless the context genuinely fits\n"
...
def build_image_prompt_prompt(
    *,
    ...
    return (
        ...
        f"Prompt library version: {PROMPT_LIBRARY_VERSION}\n\n"
        f"{_prompt_sections('brand', 'voice', 'content_rules', 'banned_phrases', 'shareable_food_post_rules', 'image_generation')}\n\n"
        f"Content slot: {content_slot}\n\n"
        f"Brand rules:\n{_brand_rules_text(brand_rules)}\n\n"
        f"Privacy-safe internal snapshot:\n{internal_snapshot}\n\n"
        f"Privacy-safe external context:\n{external_context}\n\n"
        ...
        "If the creative includes text overlay or caption space, leave room for short share-trigger copy rather than abstract jokes."
    )
