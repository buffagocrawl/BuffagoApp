...
def _weekly_body(*, runs: list[dict[str, Any]], posts: list[dict[str, Any]], metrics: list[dict[str, Any]], errors: list[dict[str, Any]], context: dict[str, Any], costs: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        *worst_lines,
        ...
        "Best video assets: " + (", ".join(_named_items(context.get("best_video_assets"))) or "No video asset data yet."),
        "Worst video assets: " + (", ".join(_named_items(context.get("worst_video_assets"))) or "No video asset data yet."),
        "Best Reel caption types: " + (", ".join(_named_items(context.get("best_caption_types"))) or "No Reel caption data yet."),
        f"Costs: estimated ${costs.get('estimated_cost_usd') if costs.get('estimated_cost_usd') is not None else 'unknown'}; tokens {costs.get('total_tokens', 0)}",
