...
def _restaurant_scene(candidate: ContentCandidate, restaurant: str, city: str) -> tuple[str, str, str, str]:
    ...
    conflict = "everyone is trying to decide who gets the last perfect glossy wing while the basket is protected like treasure"
    mood = "hungry, funny, local, and slightly overdramatic"
...
def _meme_scene(candidate: ContentCandidate) -> tuple[str, str, str, str]:
    ...
    conflict = "a ridiculous flats-versus-drums argument escalates into mock courtroom drama around the sauciest wing on the table"
    mood = "absurd, cinematic, food-obsessed, and immediately understandable without a caption"
    return setting, characters, conflict, mood
...
def _community_scene(candidate: ContentCandidate, city: str) -> tuple[str, str, str, str]:
    ...
    conflict = "the group cannot agree on the next stop because the current wings look too good to leave behind"
    mood = "communal, playful, active, and appetizing"
    return setting, characters, conflict, mood
...
def _challenge_scene(candidate: ContentCandidate, city: str) -> tuple[str, str, str, str]:
    ...
    conflict = "one person proposes an impossible extra wing stop and the table reacts like a championship call just happened"
    mood = "competitive, playful, kinetic, and wing-first"
    return setting, characters, conflict, mood
