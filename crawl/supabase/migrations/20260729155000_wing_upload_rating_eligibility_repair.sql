-- Forward-only repair for the Wing Shot upload eligibility boundary.
-- destination_ratings has no provenance columns; the append-only receipt is
-- the persisted proof created by the verified in-person rating RPC.
...
begin
  select * into v_rating
   ...
   where id = p_rating_id and user_id = p_user_id;
  if not found then return 'rating_not_owned'; end if;
  ...
  if not found then
    if coalesce(v_rating.is_buffacoin, false) then return 'buffacoin_rating'; end if;
    return 'in_person_not_verified';
  end if;
  if v_receipt.eligibility_reason = 'onboarding_seed'
     or v_receipt.verification_type = 'onboarding_seed' then
    return 'onboarding_rating';
  end if;
  if coalesce(v_rating.is_buffacoin, false) then
    return 'buffacoin_rating';
  end if;
  if v_receipt.verification_type <> 'in_person_proximity'
     or not v_receipt.wing_shot_eligible
     or v_receipt.eligibility_reason <> 'verified_in_person' then
    return 'in_person_not_verified';
  end if;
  -- Retain the existing completed-rating, destination, crawl-owner, and
  -- receipt-identity checks; this repair only makes their failure observable.
  if not public.wing_shot_rating_is_eligible(p_rating_id, p_user_id) then
    return 'in_person_not_verified';
  end if;
  return 'eligible';
end;
...
begin
  ...
  if p_rating_id is not null then
    -- Keep the lookup explicitly bound to both the supplied rating and the
    -- authenticated owner. Do not use crawl_id as an in-person signal.
    select * into v_rating
      from public.destination_ratings rating
     where rating.id = p_rating_id
       and rating.user_id = auth.uid();
    if not found then
      if exists (select 1 from public.destination_ratings rating where rating.id = p_rating_id) then
    ...
    v_reason := public.wing_shot_rating_eligibility_reason(
      p_rating_id, auth.uid(), p_destination_id
    );
  ...
  if p_expected_mime_type not in ('image/jpeg','image/png','image/webp','image/heic','video/mp4','video/quicktime') then
  ...
  v_fingerprint := md5(concat_ws('|', coalesce(p_rating_id::text,''), p_media_type, p_expected_mime_type, p_expected_size_bytes::text, p_consent_version, p_attribution_preference, coalesce(p_user_caption,''), p_destination_id::text, p_submission_source));
  ...
  values (v_user_id,v_submission_id,'reserve_upload',p_idempotency_key,v_fingerprint,v_result,p_correlation_id);
  return v_result;
end;
$$;

revoke all on function public.wing_shot_rating_eligibility_reason(uuid,uuid,uuid) from public, anon, authenticated;
grant execute on function public.wing_shot_rating_eligibility_reason(uuid,uuid,uuid) to service_role;
