...
const migration = readFileSync(
  new URL('../supabase/migrations/20260729132000_wing_creator_surfaces.sql', import.meta.url),
  'utf8'
);
const client = readFileSync(new URL('../lib/wingCreator.js', import.meta.url), 'utf8');
const history = readFileSync(
  new URL('../app/profile/wing-shots/index.jsx', import.meta.url),
  'utf8'
);
const detail = readFileSync(
  new URL('../app/profile/wing-shots/[id].jsx', import.meta.url),
  'utf8'
);
const leaderboard = readFileSync(
  new URL('../components/creator/CreatorLeaderboardPanel.jsx', import.meta.url),
  'utf8'
);
const edge = readFileSync(
  new URL('../supabase/functions/wing-media-preview/index.ts', import.meta.url),
  'utf8'
);
...
test('owner detail is private, sanitized, and exposes only real posted HTTPS links', () => {
  ...
  const detailFunction = migration.slice(
    migration.indexOf('create or replace function public.get_my_wing_submission_detail'),
    migration.indexOf('revoke all on function public.get_my_wing_submission_detail')
  );
  const returnedColumns = detailFunction.slice(
    detailFunction.indexOf('returns table'),
    detailFunction.indexOf('language sql')
  );
...
test('Creator leaderboard is server gated and keeps privacy filtering in authoritative RPC', () => {
  ...
  assert.match(
    migration,
    /revoke execute on function public\.get_wing_creator_leaderboard\(text, integer\)\s+from authenticated/
  );
  assert.match(client, /get_wing_creator_leaderboard_surface/);
  assert.match(leaderboard, /Ranked by approved Creator XP—not upload count/);
  ...
  for (const label of [
    ...
    assert.ok(
      migration.includes(`'${label}'`) ||
        readFileSync(
          new URL('../components/creator/SubmissionStatusChip.jsx', import.meta.url),
          'utf8'
        ).includes(label)
    );
  }
