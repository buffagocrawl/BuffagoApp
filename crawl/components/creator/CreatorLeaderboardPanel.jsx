...
export default function CreatorLeaderboardPanel({ active = true }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRows(nextRows);
      trackEvent({
        eventName: 'creator_leaderboard_viewed',
        screen: 'social_leaderboards',
        metadata: { period, row_count: nextRows.length },
      });
    } catch (loadError) {
      setChecked(true);
      setError(loadError?.message || 'Could not load the Creator leaderboard.');
    } finally {
      setLoading(false);
    }
  }, [active, period]);
  ...
  return (
    ...
    >
      <Card.Content style={styles.content}>
          ...
          buttons={[
            {
              value: 'week',
              label: 'This Week',
              testID: 'creator.leaderboard.period.week',
              accessibilityLabel: 'Show weekly Creator leaderboard',
            },
            {
              value: 'all_time',
              label: 'All Time',
              testID: 'creator.leaderboard.period.all-time',
              accessibilityLabel: 'Show all-time Creator leaderboard',
            },
          ]}
        />
        ...
        {loading ? (
          <View
            testID="creator.leaderboard.loading"
            style={styles.loading}
            accessibilityLabel="Loading Creator leaderboard"
          >
            <ActivityIndicator />
          </View>
        ) : error ? (
          <View testID="creator.leaderboard.error" style={styles.error}>
                ...
                <Pressable
                  ...
                  onPress={() =>
                    router.push({
                      ...
                      params: {
                        userId: row.user_id,
                        sourceSurface: 'creator_leaderboard',
                      },
                    })
                  }
                >
