...
|---|---|---|---|---|---|---|---|---|
| WS-01 | Product architect | — | `docs/wing-shots/*` | IN_PROGRESS | — | contract audit | G1 pending Serrano | Contract must match migrations |
...
| WS-03 | Database/schema architect | 01-02 | new `crawl/supabase/migrations/*wing_shots_core.sql` | READY | — | migration/state | G1 pending | Schema breadth |
| WS-04 | Rating provenance architect | 03 | new rating-verification migration/RPC | READY | — | eligibility matrix | G1 pending | Home/crawl migration |
| WS-05 | Supabase RLS/security specialist | 03-04 | new RLS/RPC migration | PLANNED | — | live role matrix | G3 pending | No current admin roles |
| WS-06 | Storage architect | 03,05 | storage migration/policies | PLANNED | — | storage policies | G3 pending | Private Meta ingestion |
| WS-07 | Feature-flag engineer | 03 | flags migration/config/docs | PLANNED | — | rollout tests | G1 pending | Server/client drift |
...
| WS-15 | Badge and XP engineer | 03-04 | creator rewards/badges migration | PLANNED | — | idempotency/forgery | G7 pending | Generic XP forgery |
...
| WS-19 | Media-processing worker engineer | 03,06 | `Agents/Jalapeno/wing_shots/processing/**` | PLANNED | — | lease/retry/DLQ | G3 pending | Worker runtime |
| WS-20 | FFmpeg/video engineer | 19 | video processor | PLANNED | — | ffprobe no-audio | G3/G4 pending | Codec portability |
| WS-21 | Image-processing engineer | 19 | image processor | PLANNED | — | EXIF/magic/crop | G3/G4 pending | Dependency availability |
...
| WS-34 | Account-deletion engineer | 03,06,15 | delete-account function/migration | PLANNED | — | erasure/audit | G3 pending | Auth-last migration |
...
|---|---|---|---|---|
| Durable in-person provenance absent | Critical | WS-04 | OPEN | eligibility RPC and matrix tests |
| Authenticated generic XP forgery | Critical | WS-15 | OPEN | revoked/allowlisted grants and abuse test |
| Reusable admin role model absent | Critical | WS-05 | OPEN | role migration and non-admin denial |
| Existing Jalapeño buckets public | High | WS-06/30 | OPEN | private bucket/RLS and no reuse |
| Auth-first deletion omits media | High | WS-34 | OPEN | erasure workflow test |
...
| Guest ownership contract unresolved | High | WS-08/10 | OPEN | eligible receipt requires authenticated owner |
